...
const HelpCategory: FC<HelpCategoryProps> = ({ mainImage = {}, heading = '', categories = [], whatsappAe = '+966553561501', whatsappEg = '+201212777724', whatsappSa = '+966553561501' }) => {
    ...
    const handleLinks = () => {
        if (appState?.region == 'sa') {
            ...
            router.push(`https://web.whatsapp.com/send?phone=${whatsappEg}&text=Hello!%20I%20am%20interested%20in%20your%20product`)
        }
    }
                    ...
                    {categories && categories.length > 0 && categories.map((category, index) => {
                        ...
                        return (
                            <div className={styles['category-section']} style={{ backgroundColor: width > desktopScreenSize ? bgColor : mobileBgColor }}>
                                ...
                                {
                                    details &&
                                    <Label className={styles['category-details']}>{regionDetails}</Label>
                                }
                                {
                                    imageText &&
                                    <div className={styles['category-imageBlock']} onClick={() => { handleLinks() }}>
                                        <Whatsapp />
